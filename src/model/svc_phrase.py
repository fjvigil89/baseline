...
def svc_Model(count_fold=5, c_value_init=1, c_value_end=1, degree_value_init=1, degree_value_end=1 ):
    # # ==============================================================================            
    data_processing = {
        'list':[],
        'model':[],
    ...
    while nfold <= count_fold:   
        ...
        _c=[ 2**x for x in range(c_value_init, c_value_end)]          
        # length=range(c_value_init,3)
        for c in _c:
            _uar=list()
            _precision = list()
            _f1Score=list()
            ...
            #degree=[ 2**x for x in range(degree_value_init, degree_value_end)]
            degree=range(1,5)
            for deg in degree:                  
                modelo.C = c
                modelo.degree = deg                
...
def rbf_Model(count_fold=5, c_value_init=1, c_value_end=1, gamma_value_init=1, gamma_value_end=1):
    # # ==============================================================================            
    data_processing = {
        'list':[],
        'model':[],
    ...
    while nfold <= count_fold:                
        ...
        for c in _c:
            ...
            _recall=list()
            _average_precision=list()
            gamma=[ 2**x for x in range(gamma_value_init, gamma_value_end)]
            for gam in gamma:  
                modelo.C = c
                modelo.gamma = gam                
                modelo.fit(X_train, y_train)
